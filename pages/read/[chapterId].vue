...
<template>
  <div class="min-h-screen bg-black text-zinc-100 flex flex-col justify-between">
    <!-- Top Sticky Floating Header -->
    <header
      :class="[
        'fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-b border-zinc-800/80 transition-transform duration-300 px-4 sm:px-8 py-3 flex items-center justify-between gap-4',
    ...
    >
      ...
      <div class="flex items-center gap-3.5 min-w-0">
        <NuxtLink
          ...
          class="p-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 flex-shrink-0 transition-colors"
        ...
        <div class="min-w-0">
          <h2 class="text-sm sm:text-base font-bold truncate text-white">
            {{ parentManga?.title || 'Чтение манги' }}
          </h2>
          <p class="text-xs sm:text-sm text-zinc-400 truncate font-medium">
            Глава {{ chapterInfo?.chapter || '...' }}
            <span v-if="chapterInfo?.name"> — {{ chapterInfo.name }}</span>
          </p>
        </div>
      </div>

      <!-- Center: Quick Prev/Next -->
      ...
      <!-- Right: Reader Settings Controls -->
      <div class="flex items-center gap-2 relative">
        <button
          type="button"
          class="p-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 transition-colors"
        ...
        <div
          v-if="isSettingsOpen"
          class="absolute right-0 top-full mt-2 w-80 bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl p-5 z-50 text-sm space-y-5"
        >
          <!-- Mode (Webtoon / Single) -->
          <div>
    ...
    <main class="flex-1 pt-20 pb-20">
      ...
      <div v-else-if="error" class="min-h-[60vh] flex items-center justify-center p-4">
        <div class="max-w-md w-full text-center space-y-4 bg-zinc-950 border border-zinc-800 p-8 rounded-3xl shadow-2xl">
          <AlertCircle class="w-12 h-12 text-zinc-500 mx-auto" />
          <h2 class="text-xl font-bold text-zinc-200">Не удалось загрузить главу</h2>
          ...
          <div class="flex justify-center gap-3 pt-2">
            ...
            <NuxtLink
              ...
              class="px-6 py-3 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 text-sm font-bold border border-zinc-800"
            >
              К тайтлу
            </NuxtLink>
          </div>
      ...
      <div v-else-if="chapterInfo?.isPaid || pages.length === 0" class="min-h-[60vh] flex items-center justify-center p-4">
        <div class="max-w-lg w-full text-center space-y-6 bg-zinc-950 border border-amber-500/30 p-8 sm:p-10 rounded-3xl shadow-2xl">
          <div class="w-16 h-16 rounded-2xl bg-amber-500/10 text-amber-400 border border-amber-500/30 flex items-center justify-center mx-auto">
            <Lock class="w-8 h-8" />
          </div>
          <div class="space-y-2">
            <h2 class="text-2xl font-black text-white">Платная глава</h2>
          ...
          <div class="flex flex-col sm:flex-row gap-3 justify-center pt-2">
            <a
              ...
              class="px-7 py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-sm font-black flex items-center justify-center gap-2 transition-colors shadow-lg"
            >
            ...
            <button
              v-if="nextChapter"
              type="button"
              class="px-6 py-3.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-200 text-sm font-bold border border-zinc-800 transition-colors"
      ...
      <div v-else>
        ...
        >
          <div
            ...
            :key="index"
            class="w-full relative min-h-[400px] flex items-center justify-center bg-zinc-950"
          >
            <div v-if="failedImages[index]" class="py-24 text-center space-y-3">
              <p class="text-sm text-zinc-500">Не удалось загрузить страницу {{ index + 1 }}</p>
              <button
                type="button"
        ...
        <div
          v-else
          :class="['mx-auto flex flex-col items-center select-none cursor-pointer', widthClass]"
          @click="nextPage"
        >
          <div class="w-full relative min-h-[600px] flex items-center justify-center bg-zinc-950">
            <div v-if="failedImages[currentPageIndex]" class="py-24 text-center space-y-3">
              <p class="text-sm text-zinc-500">Не удалось загрузить страницу {{ currentPageIndex + 1 }}</p>
              <button
                type="button"
                class="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold rounded-lg flex items-center gap-2 mx-auto"
        ...
        <div class="max-w-xl mx-auto px-4 py-12 flex flex-col items-center gap-6">
          <div class="flex items-center gap-3 w-full">
            <button
              type="button"
              :disabled="!prevChapter"
              class="flex-1 py-4 px-6 rounded-2xl bg-zinc-900 hover:bg-zinc-800 text-zinc-200 hover:text-white font-bold text-sm sm:text-base border border-zinc-800 disabled:opacity-30 disabled:pointer-events-none flex items-center justify-center gap-2 transition-all shadow-lg"
              @click="prevChapter && goToChapter(prevChapter.id)"
            >
              <ChevronLeft class="w-5 h-5" />
            ...
            <button
              type="button"
              :disabled="!nextChapter"
              class="flex-1 py-4 px-6 rounded-2xl bg-white hover:bg-zinc-200 text-zinc-950 font-black text-sm sm:text-base disabled:opacity-30 disabled:pointer-events-none flex items-center justify-center gap-2 transition-all shadow-xl"
              @click="nextChapter && goToChapter(nextChapter.id)"
            >
